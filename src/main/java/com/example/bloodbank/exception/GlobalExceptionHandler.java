...
public class GlobalExceptionHandler {
	...
	// Throw exception when blood group is not valid.
	...
	// Throw exception when request isn't valid.
	...
	public ResponseEntity<ErrorResponse> handleInvalidDonorId(SQLIntegrityConstraintViolationException ex) {
		ErrorResponse errorResponse = new ErrorResponse(HttpStatus.UNPROCESSABLE_ENTITY.value(),
				"Donor is not registered");
		return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY).body(errorResponse);
	}

	// Throw exception when validations are not fulfilled.
	...
	public ResponseEntity<Map<String, List<String>>> handleConstraintViolationException(
		...
		List<String> errors = ex.getConstraintViolations().stream().map(t -> t.getMessage())
				.collect(Collectors.toList());
		ErrorResponse errorResponse = new ErrorResponse(HttpStatus.UNPROCESSABLE_ENTITY.value(),
	...
	// Throw exception when invalid IDs are passed in request.
