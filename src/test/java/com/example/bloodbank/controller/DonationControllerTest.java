...
public class DonationControllerTest {
	...
	private void setup() {
		...
		mockMvc = MockMvcBuilders.standaloneSetup(donationController).setControllerAdvice(new GlobalExceptionHandler())
				.build();
	}
	...
	public void whenPostRequestToDonationAndValidDonation_thenCorrectResponse() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.post("/addDonation").contentType(MediaType.APPLICATION_JSON)
				.content(donationRequest)).andExpect(MockMvcResultMatchers.status().isOk())
				.andExpect(MockMvcResultMatchers.content().contentType(MediaType.APPLICATION_JSON))
				.andExpect(jsonPath("$", aMapWithSize(5)))
				.andExpect(jsonPath("$.donorId", is(donation.getDonorId().toString())));
	}
	...
	public void whenPostRequestToDonationAndInvalidDonation_thenThrowException() throws Exception {
		...
		Mockito.when(donationService.lastDonationMade(Mockito.any()))
				.thenThrow(new ValidationException("Donation has been made in last 7 days."));
		mockMvc.perform(MockMvcRequestBuilders.post("/addDonation").contentType(MediaType.APPLICATION_JSON)
				.content(donationRequest)).andExpect(MockMvcResultMatchers.status().is4xxClientError());
	}
	...
	public void whenPutDonationAndValidDonation_thenCorrectResponse() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.put("/modifyDonation/{id}", donationId)
				.contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(donation)))
				.andExpect(jsonPath("$", aMapWithSize(5)))
